...
user_form_wizard() {
    ...
    while true; do
        ...
        while [[ -z "$SYS_USER" || ! "$SYS_USER" =~ ^[a-z_][a-z0-9_-]*$ ]]; do
            ...
            echo
            if [[ -n "$user_err" ]]; then
                say --foreground 1 "$user_err"
                echo
            fi
            SYS_USER=$(g_input --placeholder "Solo minúsculas y números (ej: usuario)" --prompt.foreground="#845DF9" --prompt "Usuario> ")
            SYS_USER="${SYS_USER// /}"
            if [[ -z "$SYS_USER" ]]; then
                user_err="El nombre de usuario no puede estar vacío."
            elif [[ "$SYS_USER" =~ [A-Z] ]]; then
                user_err="El usuario no debe contener mayúsculas. Solo minúsculas y números (ej: ${SYS_USER,,})."
            elif [[ ! "$SYS_USER" =~ ^[a-z_][a-z0-9_-]*$ ]]; then
                user_err="El usuario debe comenzar con letra minúscula y contener solo minúsculas, números o guiones."
            else
                user_err=""
            fi
        done
        ...
        while true; do
            ...
            if [[ -z "$MASTER_PASS" ]]; then
                pass_err="La contraseña no puede estar vacía. Inténtalo de nuevo."
            elif [[ "$MASTER_PASS" != "$pass_confirm" ]]; then
                pass_err="Las contraseñas no coinciden. Inténtalo de nuevo."
            else
                pass_err=""
                break
            fi
...
perform_installation_worker() {
    ...
    BOOT_ENTRY_OPTIONS="cryptdevice=UUID=$ROOT_UUID:cryptroot root=/dev/mapper/cryptroot rootflags=subvol=@ rw quiet splash loglevel=3 rd.udev.log_level=3 vt.global_cursor_default=0"
    ...
    # Silenciar hook de btrfs para que no ensucie la pantalla con mensajes de escaneo
    cat << 'BTRFS_HOOK' > /mnt/usr/lib/initcpio/hooks/btrfs
#!/usr/bin/ash
run_hook() {
...
cat << 'UDEV_BACKLIGHT' > /etc/udev/rules.d/90-backlight.rules
ACTION=="add", SUBSYSTEM=="backlight", RUN+="/bin/chmod a+rw /sys/class/backlight/%k/brightness"
UDEV_BACKLIGHT
...
if ! command -v yay >/dev/null 2>&1; then
    ...
    su - "$SYS_USER" -c "
        ...
        rm -rf /tmp/yay-bin
    " || {
        echo "Aviso: Intentando compilar yay desde fuentes..."
        su - "$SYS_USER" -c "
            rm -rf /tmp/yay
...
# Instalar la versión más reciente de Zen Browser desde AUR usando yay
echo "Instalando Zen Browser con: yay -S zen-browser-bin..."
su - "$SYS_USER" -c "yay -S --noconfirm --needed zen-browser-bin" || {
    echo "Aviso: reintentando instalación manual de zen-browser-bin..."
    su - "$SYS_USER" -c "
        rm -rf /tmp/zen-browser-bin
        git clone --depth 1 https://aur.archlinux.org/zen-browser-bin.git /tmp/zen-browser-bin && \
...
# Restaurar permisos normales de sudo (con contraseña obligatoria)
echo "%wheel ALL=(ALL:ALL) ALL" > /etc/sudoers.d/wheel
chmod 440 /etc/sudoers.d/wheel

# Enlace simbólico de conveniencia /usr/bin/zen
if [ -f /usr/bin/zen-browser ] && [ ! -f /usr/bin/zen ]; then
    ln -sf /usr/bin/zen-browser /usr/bin/zen 2>/dev/null || true
elif [ -f /usr/bin/zen ] && [ ! -f /usr/bin/zen-browser ]; then
...
{
  "path": "/home/$SYS_USER/Pictures/Wallpapers/default.jpg",
...
GTK4_CONF
    ...
    cat << 'PROF_BIN' > /mnt/etc/profile.d/local_bin.sh
export PATH="$HOME/.local/bin:/usr/local/bin:$PATH"
PROF_BIN
    chmod 644 /mnt/etc/profile.d/local_bin.sh
...
selection_background #282d38

cursor #88c0d0
cursor_text_color #1a1d24

active_border_color #88c0d0
inactive_border_color #353b49
active_tab_background #88c0d0
active_tab_foreground #1a1d24
inactive_tab_background #14161d
